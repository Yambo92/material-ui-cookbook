...
export default function DrawerItemState() {
    ...
    /* This is a higher-order function that takes the
    label component to display as an argument and returns a new function that
    will update the content when the item is clicked on. */
    const onClick = content => () => { //避免立即执行然后陷入无限循环
        setOpen(false);
        setContent(content)
    };
    return (
